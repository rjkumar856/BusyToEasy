...
if(isset($_POST['cus_add_new_user'])){
    try{
		...
		if(empty($question_set) || empty($subjects) || empty($question_title) || empty($option1) || empty($option2) || empty($correct_answer) || empty($answer_description)){
		    ...
		    $stmt = $reg_user->runQuery("UPDATE online_test_questions_prelims SET question_type='$question_set',subject_id='$subjects',question_title='$question_title',option1='$option1',option2='$option2',
		    option3='$option3',option4='$option4',correct_answer='$correct_answer',answer_description='$answer_description' WHERE id='$edit_requested_id'");
            $stmt->execute();
            
    		$_SESSION['add_user_error_msg'] = "<div class='alert alert-success'><button class='close' data-dismiss='alert'>&times;</button><strong>Prelims Question!</strong> Updated Successfully.</div>";
    		header("Location: edit-prelims-question?id=".$edit_requested_id);
    		exit();
		}
...
$edit_requested_query = $reg_user->runQuery("SELECT *, otq.id as QID FROM online_test_questions_prelims otq INNER JOIN online_test_subjects ots ON ots.id=otq.subject_id WHERE otq.id='$edit_requested_id' ");
...
<div class="site-content">
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Edit Prelims Questions</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="view-prelims-questions">View Prelims Questions</a></li>
							<li class="breadcrumb-item active"><a href="#">Edit Prelims Questions</a></li>
						</ol>
							...
							<form class="form-horizontal" method="post" name="edit_ad" enctype="multipart/form-data">
								<div class="row">
									...
									<div class="form-group">
										...
										<input type="number" id="question_set" placeholder="Question Set Number" name="question_set" value="<?php echo str_replace(array("<br>","<br />","<br >","<br/>"),PHP_EOL,$edit_requested_details['question_type']); ?>" class="form-control" required>
									</div>
									<div class="form-group">
										...
										<?php
                                				...
                                				<option <?php if($edit_requested_details['subject_id'] == $stmt1->id){ echo "selected"; }else{ echo ""; } ?> value="<?php echo $stmt1->id; ?>"><?php echo $stmt1->title; ?></option>
                                	    <?php } ?>
                                			</select>
                                	</div>
								...
								<div class="col-md-12">
									<div class="pull-left">
									<input type="submit" class="btn btn-primary" value="Update Prelims Question" name="cus_add_new_user">
									</div>
									</div>
						</div>
							</form>

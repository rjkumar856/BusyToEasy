...
$edit_requested_id=$_GET['id'];
    ...
    header("Location: view-prelims-results");
}
...
<div class="content-area py-1">
	<div class="container-fluid">
		<h4>Prelims Results Details</h4>
		<ol class="breadcrumb no-bg mb-1">
			<li class="breadcrumb-item"><a href="view-prelims-results">View Prelims Results</a></li>
			<li class="breadcrumb-item active">View Prelims Results Details</li>
		</ol>
		<div class="box box-block bg-white content quiz">
		<?php
		$count=1;
        $stmt = $reg_user->runQuery("SELECT *, qr.id as RestID, us.id as UsrID FROM online_test_submit_prelims qr LEFT JOIN user us ON us.id=qr.user_id WHERE qr.id='$edit_requested_id' ORDER BY qr.date_added DESC");
        $stmt->execute();
        for($i=0; $stmt1 = $stmt->fetchObject(); $i++){ ?>
			<h2 class="ng-binding"><?php echo $stmt1->full_name ."(".$stmt1->UsrID.")";?> Prelims Results</h2>
			...
			$stmt_result = $reg_user->runQuery("SELECT * FROM online_test_questions_prelims WHERE id='$value'");
			$stmt_result->execute();

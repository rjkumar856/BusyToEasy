...
if(isset($_POST['SubmitButton'])){
    try{
		...
		if(empty($title) || $_FILES["result_file"]["size"]<1){
		    $_SESSION['add_user_error_msg'] = "<div class='alert alert-danger'><button class='close' data-dismiss='alert'>&times;</button>Please Fill all the Fileds!</div>";
		}else{
		    
		    $target_dir = "../assets/files/gallery/";
            ...
            $FileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                if($FileType == 'jpg' || $FileType == 'jpeg' || $FileType == 'png' || $FileType == 'gif'){
                    ...
                    if (move_uploaded_file($_FILES["result_file"]["tmp_name"], $target_file_new)){
                        ...
                        $stmt = $reg_user->runQuery("INSERT INTO galleries(title,image) 
            		    VALUES('$title','$file_name')");
                        $stmt->execute();
                        
                		$_SESSION['add_user_error_msg'] = "<div class='alert alert-success'><button class='close' data-dismiss='alert'>&times;</button><strong>New Gallery Images!</strong> Added Successfully.</div>";
...
<div class="site-content">
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Create New Gallery Image</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="view-gallery">Gallery Image List</a></li>
							<li class="breadcrumb-item active"><a href="#">Add Gallery Image</a></li>
						</ol>
						<div class="box box-block bg-white">
							...
							<form name="news upload" id="news" action="" method="POST" enctype="multipart/form-data">
								<div class="form-group">
									...
									<input type="text" class="form-control" name="title" id="title" value="<?php if(isset($_POST['title'])){ echo $_POST['title']; } ?>" placeholder="Title" required="required"/>
								</div>
								<div class="form-group">

...
if(isset($_GET['delete_id'])){
	$stmt = $reg_user->runQuery("DELETE FROM daily_article WHERE id=".$_GET['delete_id']);
	$stmt->execute(array(":id"=>$id));
	header("Location: view-article-daily-news");
}
...
<div class="site-content">
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Daily News Article List</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="#">Home</a></li>
							<li class="breadcrumb-item active">View Daily News Article</li>
						</ol>
						<div class="box box-block bg-white">
							<h5 class="mb-1">Exporting Daily News Article Data</h5>
							<table class="table jsgrid table-striped table-bordered dataTable table-responsive" id="table-2">
								...
								<?php
								$count=1;
                                        $stmt = $reg_user->runQuery("SELECT * FROM daily_article ORDER BY date_added DESC");
                                        $stmt->execute();
                                        for($i=0; $stmt1 = $stmt->fetchObject(); $i++){
                                        ?><tr>	
                                        <td class="jsgrid-cell jsgrid-control-field jsgrid-align-center" style="width: 60px;">
                                            <a href="edit-daily-news-article?id=<?php echo $stmt1->id;?>"><button class="jsgrid-button jsgrid-edit-button" type="button" title="Edit"></button></a>
    									...
    									<td><?php if(!empty($stmt1->file_links)){?><a href="../assets/files/article/<?php echo $stmt1->file_links;?>" target="_blank"><i class="fa fa-download" aria-hidden="true"></i> Download</a><?php } ?></td>
		...
		function delete_id(id){
		    if(confirm('Are you sure you want to Delete this Daily News?')){
		        window.location.href='view-article-daily-news?delete_id='+id;
		    }
		}
		</script>
		</body>

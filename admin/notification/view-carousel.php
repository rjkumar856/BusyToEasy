...
if(isset($_GET['delete_id'])){
	...
	$stmt = $reg_user->runQuery("DELETE FROM carousel_images WHERE id=".$_GET['delete_id']);
	$stmt->execute(array(":id"=>$id));
	header("Location: view-carousel");
}
					...
					<div class="container-fluid">
						...
						<div class="box box-block bg-white">
							...
							<table class="table jsgrid table-striped table-bordered dataTable table-responsive" id="table-2">
								...
								<tbody>
                                    <?php
                                    $stmt = $reg_user->runQuery("SELECT * FROM carousel_images ORDER BY id");
                                    $stmt->execute();
									$counter = 0;
                                    ...
                                    <td class="jsgrid-cell jsgrid-control-field jsgrid-align-center" style="width: 50px;">
											...
											<td><?php echo $stmt1->title1; ?></td>
    										<td><?php echo $stmt1->title2; ?></td>
    										<td><?php echo $stmt1->title3;?></td>
    										<td><?php echo $stmt1->file_url;?></td>
    										<td><?php echo $stmt1->image;?></td>
										</tr><?php } ?>
								</tbody>
							</table>
						</div>
...
function delete_id(id)
{
	...
	{
		window.location.href='view-carousel?delete_id='+id;
	}
}
</script>
...
if(isset($_GET['delete_id'])){
	...
	$stmt = $reg_user->runQuery("DELETE FROM push_notification WHERE id=".$_GET['delete_id']);
	$stmt->execute(array(":id"=>$id));
	header("Location: view-mobile-notification");
}
					...
					<div class="container-fluid">
						...
						<div class="box box-block bg-white">
							...
							<table class="table jsgrid table-striped table-bordered dataTable table-responsive" id="table-2">
								...
								<tbody>
                                    <?php
                                    $stmt = $reg_user->runQuery("SELECT * FROM push_notification ORDER BY date_added DESC");
                                    $stmt->execute();
									$counter = 0;
...
function delete_id(id)
{
	...
	{
		window.location.href='view-mobile-notification?delete_id='+id;
	}
}
</script>